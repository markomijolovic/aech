...
public:
    ...
    template <typename T>
    auto set_signature(signature_t signature) -> void
    {
        const auto &type_info  = typeid(std::remove_reference_t<T>);
        const auto &type_index = std::type_index{type_info};
    ...
    auto entity_destroyed(entity_t entity) -> void;

    auto entity_signature_changed(entity_t entity, signature_t entity_signature) -> void;
