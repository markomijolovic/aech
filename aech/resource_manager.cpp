...
auto load_shader(const std::string &name,
                 ...
                 const std::string &geometry) -> shader_t &
{
...
auto load_hdr_texture(const std::string &name, const std::string &path) -> texture_t *
{
    if (textures.find(name) != std::end(textures)) {
        return &textures[name];
    }
...
auto parse_mesh(aiMesh *mesh, const aiScene * /*scene*/) -> mesh_t *
{
...
auto load_texture_cube(const std::string &name, const std::string &folder) -> texture_cube_t *
{
...
auto parse_material(const aiScene * /*scene*/, aiMaterial *a_material) -> material_t *
{
