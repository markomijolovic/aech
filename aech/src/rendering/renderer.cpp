...
renderer_t::renderer_t() noexcept
{
    ...
    engine.add_component(dirlight, directional_light_t{&engine.get_component<transform_t>(dirlight), {1, 1, 1}, 5});
    ...
    auto &camera_transform = engine.get_component<transform_t>(m_camera_entity);
    ...
    for (auto x = -13.5F; x <= 14.F; x += 4.2F) {
        for (auto y = -2.5F; y <= 15.F; y += 4.2F) {
            for (auto z = -4.6F; z <= 5.5F; z += 4.2F) {
                const auto probe1 = engine.create_entity();
                engine.add_component(probe1, transform_t{{x, y, z}, {}, {8.4F, 8.4F, 8.4F}});
    ...
    const std::array  capture_views{
        ...
        look_at({}, math::vec3_t{0, 0, -1}, {0, -1, 0})};
    const auto skybox = resource_manager::load_hdr_texture("skybox", "resources/hdr/skybox.hdr");
...
auto renderer_t::bake_probes() const noexcept -> void
{
    gi_renderer->bake_probes();
}

auto renderer_t::options() const noexcept -> bool
{
    return m_options;
}

auto renderer_t::sort_front_to_back(entity_t a, entity_t b) noexcept -> bool
{
...
auto renderer_t::sort_back_to_front(entity_t a, entity_t b) noexcept -> bool
{
    auto &     scene_node_a = engine.get_component<scene_node_t>(a);
    auto &     scene_node_b = engine.get_component<scene_node_t>(b);
    const auto aabb_a       = scene_node_a.bounding_box();
...
auto renderer_t::set_options(bool gui) noexcept -> void
{
    m_options = gui;
}

auto renderer_t::poisson_sampling_distance() const noexcept -> float
{
    return m_poisson_sampling_distance;
}

auto renderer_t::ssao() const noexcept -> bool
{
    return m_ssao;
}

auto renderer_t::render_cache() noexcept -> render_cache_t *
{
    return &m_render_cache;
}

auto renderer_t::ssao_texture() const noexcept -> const texture_t *
{
    return &m_ssao_blurred_fbo->colour_attachments().front();
}

auto renderer_t::shadows() const noexcept -> bool
{
    return m_shadows;
}

auto renderer_t::post_process() noexcept -> void
{
...
auto renderer_t::render_gui() noexcept -> void
{
...
auto renderer_t::render_ssao() noexcept -> void
{
    m_ssao_fbo->bind();
    m_render_cache.set_shader(m_ssao_shader);
    m_render_cache.set_depth_test(false);
...
auto renderer_t::update() noexcept -> void
{
