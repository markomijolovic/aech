...
auto aech::graphics::material_library::generate_default_materials() noexcept -> void
{
...
auto aech::graphics::material_library::create_material(const std::string &from) noexcept -> aech::graphics::material_t
{
    if (const auto it = default_materials.find(from); it != std::end(default_materials)) {
        return (*it).second;
    }
