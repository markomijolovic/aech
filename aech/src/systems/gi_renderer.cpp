...
aech::graphics::gi_renderer_t::gi_renderer_t(render_cache_t *render_cache, camera_t *camera) noexcept
    : m_camera{camera}
    , m_render_cache{render_cache}
{
}

auto aech::graphics::gi_renderer_t::specular_material() const noexcept -> material_t *
{
    return m_ambient_specular_material;
}

auto aech::graphics::gi_renderer_t::diffuse_material() const noexcept -> material_t *
{
    return m_ambient_diffuse_material;
}

auto aech::graphics::gi_renderer_t::render_target() const noexcept -> framebuffer_t *
{
    return m_render_target;
}

auto aech::graphics::gi_renderer_t::add_probe(reflection_probe_t &&probe) noexcept -> void
{
    m_reflection_probes.push_back(probe);
}

auto aech::graphics::gi_renderer_t::add_probe(const reflection_probe_t &probe) noexcept -> void
{
    m_reflection_probes.push_back(probe);
}

auto aech::graphics::gi_renderer_t::add_probe(light_probe_t &&probe) noexcept -> void
{
    m_light_probes.push_back(probe);
}

auto aech::graphics::gi_renderer_t::add_probe(const light_probe_t &probe) noexcept -> void
{
    m_light_probes.push_back(probe);
}

auto aech::graphics::gi_renderer_t::bake_probes() noexcept -> void
{
    ...
    aech::graphics::render_cache_t::clear(clear::color_and_depth_buffer_bit);
    m_ndc_quad->draw();
...
auto aech::graphics::gi_renderer_t::create_radiance_cubemap(math::vec3_t position, std::size_t probe_index, std::uint32_t width, std::uint32_t height) noexcept -> void
{
    m_render_cache->set_depth_test(true);
    m_render_cache->set_depth_func(depth_func::lequal);
    const std::array  capture_views{look_at(position, position + math::vec3_t{1, 0, 0}, {0, -1, 0}), look_at(position, position + math::vec3_t{-1, 0, 0}, {0, -1, 0}), look_at(position, position + math::vec3_t{0, 1, 0}, {0, 0, 1}), look_at(position, position + math::vec3_t{0, -1, 0}, {0, 0, -1}), look_at(position, position + math::vec3_t{0, 0, 1}, {0, -1, 0}), look_at(position, position + math::vec3_t{0, 0, -1}, {0, -1, 0})};
    ...
    *tex                                 = texture_cube_t{width, height, texture_types::sized_internal_format::rgb16f, texture_types::format::rgb, texture_types::type::floating_point};
...
auto aech::graphics::gi_renderer_t::create_preprocessed_environment_map(std::size_t probe_index, std::uint32_t width, std::uint32_t height) noexcept -> void
{
    m_render_cache->set_cull(false);
    ...
    *prefiltered           = texture_cube_t{width, height, texture_types::sized_internal_format::rgb16f, texture_types::format::rgb, texture_types::type::floating_point};
    ...
    m_prefilter_material->set_texture_cube("environment_map", &resource_manager::texture_cubes["radiance" + std::to_string(probe_index)], 0);
    ...
    for (std::uint32_t mip = 0; mip < levels; mip++) {
        const auto current_width  = static_cast<std::uint32_t>(height * std::pow(0.5, mip));
        const auto current_height = current_width;
        m_render_cache->set_viewport(0, 0, current_width, current_height);
...
auto aech::graphics::gi_renderer_t::create_irradiance_cubemap(std::size_t probe_index, std::uint32_t width, std::uint32_t height) noexcept -> void
{
    m_render_cache->set_cull(false);
    ...
    *irradiance           = texture_cube_t{width, height, texture_types::sized_internal_format::rgb16f, texture_types::format::rgb, texture_types::type::floating_point};

    probe.set_irradiance(irradiance);
    m_render_cache->set_shader(&m_irradiance_capture_material->shader());
    m_irradiance_capture_material->set_texture_cube("environment", &resource_manager::texture_cubes["radiance" + std::to_string(probe_index + m_reflection_probes.size())], 0);
...
auto aech::graphics::gi_renderer_t::render_ambient_pass() noexcept -> void
{
