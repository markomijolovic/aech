    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0) {
        ...
        for (int n = 0; n < components; n++) {
            ...
            if (flags & ImGuiColorEditFlags_Float) {
                value_changed |= DragFloat(ids[n], &f[n], 1.0f / 255.0f, disable_hue_edit ? +FLT_MAX : 0.0f, disable_hue_edit ? -FLT_MAX : hdr ? 0.0f
                                                                                                                                               : 1.0f,
                                           fmt_table_float[fmt_idx][n]);
                value_changed_as_float |= value_changed;
            } else {
                value_changed |= DragInt(ids[n], &i[n], 1.0f, disable_hue_edit ? INT_MAX : 0, disable_hue_edit ? INT_MIN : hdr ? 0
                                                                                                                               : 255,
