...
public:
    component_array_i()                          = default;
    component_array_i(const component_array_i &) = default;
    component_array_i(component_array_i &&)      = default;
    auto operator=(const component_array_i &) -> component_array_i & = default;
    auto operator=(component_array_i &&) -> component_array_i & = default;
...
public:
    ...
    {
        if (const auto it = m_entity_to_index.find(entity); it != std::end(m_entity_to_index)) {
            remove_data(entity);
        }
    }
