...
auto aech::input_manager_t::update(float dt) noexcept -> void
{
...
aech::input_manager_t::input_manager_t() noexcept
{
    const auto ml = [&](auto param) { return mouse_listener(param); };
    ...
    const auto kl = [&](auto param) { return keyboard_listener(param); };
...
auto aech::input_manager_t::mouse_listener(events::event_t &event) noexcept -> void
{
    m_offset = event.get_param<std::pair<float, float>>(events::window::params::mouse);
}

auto aech::input_manager_t::keyboard_listener(events::event_t &event) noexcept -> void
{
    m_buttons = event.get_param<std::bitset<32>>(events::window::params::keyboard);
}

auto aech::input_manager_t::set_camera(camera_t *camera) noexcept -> void
{
    m_camera = camera;
}
