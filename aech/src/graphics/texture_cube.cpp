...
texture_cube_t::texture_cube_t(std::uint32_t                        width,
                               ...
                               texture_types::filtering             mag) noexcept
    : m_width{width}
    , m_height{height}
...
auto texture_cube_t::operator=(texture_cube_t &&rhs) noexcept -> texture_cube_t &
{
...
auto texture_cube_t::id() const noexcept -> std::uint32_t
{
    return m_id;
}

void texture_cube_t::bind(std::int32_t unit) const noexcept
{
    if (unit >= 0) {
...
void texture_cube_t::unbind() noexcept
{
    glBindTexture(static_cast<GLenum>(texture_types::target::cube_map), 0);
}

void texture_cube_t::generate_mips() const noexcept
{
    bind();
