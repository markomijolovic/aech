...
public:
    ...
    texture_cube_t(std::uint32_t                        width,
                   ...
                   texture_types::filtering             mag    = texture_types::filtering::linear) noexcept;
    texture_cube_t(const texture_cube_t &) = delete;
    texture_cube_t(texture_cube_t &&rhs) noexcept;
    [[nodiscard]] auto operator=(const texture_cube_t &) -> texture_cube_t & = delete;
    [[nodiscard]] auto operator                                              =(texture_cube_t &&rhs) noexcept -> texture_cube_t &;
    ~texture_cube_t() noexcept;

    [[nodiscard]] auto id() const noexcept -> std::uint32_t;
    auto               generate_face(std::uint32_t index, void *data) const noexcept -> void;
    auto               generate_mips() const noexcept -> void;
